...
class Base_Segmentation:
    ...
    def segment_by_prompt_(
        ...
        if prompt.dim() == 2:
            ...
            labels = torch.ones((len(prompt), 1), device=self.device)
        else:
            labels = torch.ones((prompt.shape[:2]), device=self.device)
    ...
    def post_process_masks_notgood(
        self,
        masks,
        scores,
        depth,
        ...
        if stability_score_thresh > 0:
            stability_score = calculate_stability_score(
                masks, 0, stability_score_offset
            )
            selected_stability_idx = (
                (stability_score > stability_score_thresh).nonzero().flatten()
            )
            if len(selected_stability_idx) > 0:
                masks = masks[selected_stability_idx]
                scores = scores[selected_stability_idx]
                stability_score = stability_score[selected_stability_idx]
                seg_bbox = seg_bbox[selected_stability_idx]
                selected_area_idx = selected_area_idx[selected_stability_idx]
                scores = (scores + stability_score) / 2

        selected_nms_idx = nms_function_2(masks > mask_threshold, seg_bbox, scores)
        if len(selected_nms_idx) > 0:
            masks = masks[selected_nms_idx]
            scores = scores[selected_nms_idx]
            selected_area_idx = selected_area_idx[selected_nms_idx]
        ...
        masks = clean_small_articles(
            masks, kernel_size=int(((min_area / 10) // 2) * 2 + 1)
        )
